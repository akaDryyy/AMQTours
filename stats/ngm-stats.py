...
def observed_players_from_songs(songs):
    observed = set()
    for song in songs:
        observed.update(song.get("correctGuessPlayers", []))
        observed.update(
            list_state.get("name")
            for list_state in song.get("listStates", [])
            if list_state.get("name")
        )
    return observed
...
def run_bruteforce_stats(directory, json_dir, codes_path, teams_re, regex):
    ...
    include_usefulness = codes_are_valid and team_size is not None and team_size > 0
    usefulness_calc = Usefulness(team_size, average_rank) if include_usefulness else None
    stats = {}
    ...
    for file_name, songs in json_payloads:
        ...
        for song in songs:
            ...
            listers = {state.get("name") for state in list_states if state.get("name")}
    ...
    for row in stats.values():
        ...
        row["Usefulness"] = round(row["_usefulness_sum"] * average_rank * 2 / played, 3) if include_usefulness and played else 0.0
...
def parse_challonge_display_players(display_name):
    pattern = r"([^\s\[(]+)(?:\s*\[(.*?)\])?\s*\((.*?)\)"
    return re.findall(pattern, display_name or "")
            ...
            for side in ["player1", "player2"]:
                ...
                for masq_name, _, rank_text in parsed_players:
                    ...
                    try:
                        ...
                        raise RuntimeError(f"Missing or invalid rank for masquerade name '{masq_name}' on Challonge.")
