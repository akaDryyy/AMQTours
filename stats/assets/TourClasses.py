...
class Player:
    ...
    answerTimeTotal: float = 0.0
    answerTimeCount: int = 0
    avgAnswerTime: float = 0.0
    ...
    def post_process(self, AVGRANK, WLTcheck=True, scale_usefulness=True):
        ...
        self.avgAnswerTime = round(self.answerTimeTotal / self.answerTimeCount, 3) if self.answerTimeCount else 0.0
        if scale_usefulness:
        ...
        if (self.WIN + self.LOSE + self.TIE) == 0 and WLTcheck:
            print(self.name)
            input(
                "W-L-T check failed. This usually means a sub is missing from Challonge, "
                "a sub round is wrong, or codes.txt/Challonge disagree about who played. "
                "Fix the sub data and press Enter to exit."
            )
            exit()
