...
trueskill.setup(
    mu=10, # mean rating 
    sigma=3, # initial uncertainty of a new player's rating -- recommended to be mu/3 in docs but redefined below since most people are relatively well rated to start (?)
    beta=3, # rating difference at which the higher-rated player has a ~76% chance of winning 
    tau=0.5, # change this to increase/decrease how much a regular player's rating is likely to swing 
    draw_probability=0.01, # based on jan-jul results 
    backend='mpmath'
    )

INITIAL_SIGMA = 1.5 # change this to increase/decrease how much more a previously unseen player's rating is likely to swing compared to a regular player
...
def get_players(teamstr, elos, aliases):
    ...
    for player_str in player_strs:
        ...
        if player in elos:
            ...
            multiplier = 2 / (2 ** (math.log(min(old_tour_count + 1, 9), 3) / 2))
...
async def main():
    ...
    wks = sheet.get_worksheet_by_id(StoreElos)
